...
| `target_config`       | trace only     | `{ "window_seconds": 10..7200 }`; defaults to 1800 seconds for a trace target.        |
...
- `"target": "trace"` runs once for the whole trace after the first matching generation. Set
  `target_config.window_seconds` to control how long PostHog waits for the trace to finish before evaluating it.
...
{
  "target": "trace",
  "target_config": { "window_seconds": 1800 }
}
```
